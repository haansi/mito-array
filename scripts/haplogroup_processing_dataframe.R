...
process_haplogroup_data <- function(array_type,
  ...
  # 2. Hybrid Input Logic
  if (!is.null(array_file_path)) {
    microArray <- read.delim(array_file_path, sep = "\t", header = FALSE)
    arrayPos <- paste(microArray[[3]], collapse = ";")
  } else if (!is.null(array_df)) {
    arrayPos <- paste(array_df$Position, collapse = ";")
  } else {
  ...
  write.table(hsd_data_local, file = temp_haplogrep_input_file, sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
  ...
  cmd_distance <- paste(
    "java -jar", shQuote(jar_path_abs), " distance ",
  ...
  if (file.exists(haplogroup_distcheck_result_file)) {
    ...
    haplogroupsDiff$ArrayType <- array_type
    return(haplogroupsDiff)
