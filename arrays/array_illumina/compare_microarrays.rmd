...
groups <- combined_df %>%
  group_split(Microarray_ID)

group_ids <- combined_df %>%
  distinct(Microarray_ID) %>%
  pull()
...
ggplot(haplogroupsDiff_all, aes(x = distance, color = ArrayType)) +
  stat_ecdf(geom = "line", lwd = 1) +
  scale_x_continuous(breaks = seq(0, 25, by = 5)) + # Simplified breaks for small plots
  facet_wrap(~ArrayType, ncol = 4) + # Organizes into a grid
  labs(
    title = "Cumulative Distribution by Array Type",
    x = "Observed Haplogroup distance",
    y = "Cumulative Proportion"
  ) +
  theme_minimal() +
  theme(legend.position = "none", # Legend is redundant with facet titles
        strip.text = element_text(size = 8, face = "bold")) # Clean up facet labels
...
ggplot(haplogroupsDiff_all, aes(x = distance, color = ArrayType)) +
  ...
  labs(
    ...
    subtitle = paste("Distances on haplogroups (n=",nrow(hsd_data),") of expected vs. predicted Haplogroups"),
    x = "Observed Haplogroup distance",
    y = "Cumulative Proportion",
    color = "MicroArray"
  ) +
  theme_minimal(base_size = 12) +  coord_cartesian(xlim = c(1, max(haplogroupsDiff_all$distance, na.rm = TRUE))) + theme(legend.position = "right")
