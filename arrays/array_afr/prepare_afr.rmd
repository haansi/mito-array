...
This R-Markdown script runs over all genotyping arrays based on the publication from Lankheet et al 2021 <https://pubmed.ncbi.nlm.nih.gov/34674637/>. Thereby we calculate the haplogroups based on the present genotypes per microarray and calculate the distance to the resulting haplogroups from the sequencing data (n=193). The result is a ranked list, based on the area under the curve (AUC) for the empirical cumulative distribution function (ECDF).\

**Interpretability**: An array that has a distance of 0 for every sample would have an ECDF value of 1.0 at every point from 0 to 25, resulting in a maximum AUC of 26. The ECDF value at any distance $x$ is $P(Distance \le x)$. By averaging these probabilities across the whole range, we are calculating the expected probability that a sample falls within the "acceptable" range at any given cutoff. We use a normalized AUC (corrected by the number of points).

A score of 0.95 indicates that the array is performing at 95% of its theoretical maximum efficiency regarding haplogroup calling accuracy.
...
array_map <- c(
  "SNP array 1" = "Affymetrix Genome‑Wide Human SNP Array 6.0",
  "SNP array 2" = "Axiom Genome‑Wide Human Origins 1 Array",
  "SNP array 3" = "Axiom Genome‑Wide PanAFR Genotyping Bundle",
  "SNP array 4" = "H3Africa Array",
  "SNP array 5" = "Illumina Infinium Multi‑Ethnic AMR/AFR‑8",
  "SNP array 6" = "Illumina Infinium Multi‑Ethnic Global‑8",
  "SNP array 7" = "Illumina Infinium Omni2.5–8",
  "SNP array 8" = "Illumina Infinium Omni5–4"
)
...
auc_results <- all_distances_df %>%
  ...
  mutate(Tier = case_when(
    AUC_Normalized >= 0.90 ~ "🥇 Gold (Excellent)",
    AUC_Normalized >= 0.80 ~ "🥈 Silver (Good)",
    AUC_Normalized >= 0.70 ~ "🥉 Bronze (Fair)",
...
### 🥇 Top Performer: `r best_array_name` (Score: `r best_score`)

### ⚠️ Worst Performer: `r worst_array_name` (Score: `r worst_score`)
